...
final class PreviewViewController: UIViewController {
    ...
    private let closeButton    = PreviewViewController.makeCircleButton(symbol: "xmark")
    private let downloadButton = PreviewViewController.makeCircleButton(symbol: "arrow.down.to.line")
    ...
    // Кол-во получателей поверх фото
    ...
    private func setupViews() {
        ...
        NSLayoutConstraint.activate([
            ...
            // Badge в правом нижнем углу фото
            ...
            // Send button прикреплён к safeArea bottom
    ...
    private func configureDataSource() {
        dataSource = UICollectionViewDiffableDataSource<Section, User>(collectionView: collectionView) { [weak self] cv, indexPath, user in
            let cell = cv.dequeueReusableCell(
                withReuseIdentifier: PreviewRecipientCell.reuseIdentifier, for: indexPath
            ) as! PreviewRecipientCell
    ...
    @objc private func handleDownloadTap() {
        ...
        UIImageWriteToSavedPhotosAlbum(viewModel.asset.previewImage, self,
            #selector(imageSaved(_:didFinishSavingWithError:contextInfo:)), nil)
    }
